	...
	methods: {
		...
		async loadImages(e) {
			...
			for (let i = 0; i < this.imageFileArr[0].length; i++) {
				...
				try {
					const { DateTimeOriginal, FNumber, ISO, FocalLength, LensModel } =
						await this.getMetaData(this.imageFileArr[0][i]);
			...
			this.imageObjectArray.forEach((element, index) => {
				...
				if (
					...
					this.clusterArray.push(
						this.imageObjectArray.slice(this.prevClusterIndex, index)
					);
...
<template>
	<div
		class="w-screen h-screen flex my-auto mx-auto justify-center items-center overflow-hidden"
	>
		<ImageUploadArea
			v-if="imageUploadStage === 'uploadArea'"
			@drop.prevent="loadImages"
		...
		>
			...
			<h3 class="block">Please wait, the robot is thinking &#129302;</h3>
		</div>

		<div
			v-if="imageUploadStage === 'waitingForAccept'"
			class="flex flex-col items-center justify-center w-screen h-screen overflow-y-auto overflow-x-hidden"
		>
			<h1 class="text-5xl mb-4">Some Images Couldn't Load!</h1>
			<h3 class="mb-10">
				(This is likely due to missing metadata. Make sure your photos contain
				the DateTimeOriginal tag)
			</h3>
			<button
				class="w-auto h-auto bg-red-700 p-3 mb-20 rounded-lg"
				@click.prevent="$emit('loaded', { array: clusterArray })"
			>
				Continue Anyways
			</button>
			<div class="h-auto w-max grid grid-cols-2 items-end justify-start">
				<div
					v-for="image in unusableImages"
					:key="image.blob"
